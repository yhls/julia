...
# copy(c::CFG) = CFG(BasicBlock[copy(b) for b in c.blocks], copy(c.index)) # unused
...
#copy(code::IRCode) = IRCode(code, copy(code.stmts), copy(code.cfg), copy(code.new_nodes)) # unused
...
function add_pending!(compact::IncrementalCompact, pos::Int, attach_after::Bool)
    ...
    sort!(compact.pending_perm, DEFAULT_STABLE, Order.By(x->compact.pending_nodes.info[x].pos))
    return node
end
